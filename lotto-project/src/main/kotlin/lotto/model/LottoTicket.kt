...
data class LottoTicket(
    ...
    fun countMatch(target: LottoTicket?): Int = if (target != null) numbers.intersect(target.numbers.toSet()).size else 0

    fun countBonusMatches(target: List<Int>?): Int = if (target != null) numbers.intersect(target.toSet()).size else 0
